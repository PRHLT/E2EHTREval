...
def evalHungarian(X, Y, ctLn=0, addDum=False, fullDum=False, fcInsDel=1.0, fctRegul=0.0, sortAlign=False, reIns=False, verbTimes=False):
    ...
    if verbTimes: print("TIME CST_mat: Cost matrix building computation: %d (wrds) %f (sec)" % (lenW,timeit.default_timer()-ti), file=sys.stderr)

    # Apply Hungarian's algorithms
    # See: https://en.wikipedia.org/wiki/Hungarian_algorithm
    ti = timeit.default_timer()
    r1, r2 = linear_sum_assignment(cost_matrix)
    if verbTimes: print("TIME HUN_alg: Hungarian algorithm computation: %d (wrds) %f (sec)" % (lenW,timeit.default_timer()-ti), file=sys.stderr)
    ...
    for i, j in zip(r1, r2):
        ...
        print(f'{i:3d} {X[i]:15} ---> {j:3d} {Y[j]:15} ---> {cost_matrix[i,j]:4.2f}')
        hErr += X[i] != Y[j]
...
def main():
    ...
    parser.add_argument('-v', '--verbTimes', action='store_true', help='Print running time for each processing step.', default=False)
    parser.add_argument('-w', '--werHung', action='store_true', help='Compute Hungarian CER/WER', default=False)
    ...
    for rfile in glob.glob(os.path.join(args.dataDir, '*.'+args.refExt)):
        ...
        if verbT: print("TIME CER_lev: Levenshtein distance computation at char level: %d (chars) %f (sec)" % (lenC,timeit.default_timer()-ti), file=sys.stderr)
        ti = timeit.default_timer()
        ...
        if verbT: print("TIME WER_lev: Levenshtein distance computation at word level: %d (wrds) %f (sec)" % (lenW,timeit.default_timer()-ti), file=sys.stderr)
        # Sanity check
        ...
        if verbT: print("TIME WER_bow: Bag-of-word WER computation: %d (wrds) %f (sec)" % (lenW,timeit.default_timer()-ti), file=sys.stderr)
        ...
        print(f'\n   WER_bow: {(bow_alg-dfa)//2 + dfa:4d} {lenW:6d} {((bow_alg-dfa)//2 + df) / lenW*100:6.2f}%    ({sid})')


        if args.werHung:        
            ti = timeit.default_timer() 
            lvd_cl_alg = fastwer.compute(hyp, x, char_level=True)[0] + nic # Including inserted chars
            if verbT: print("TIME CER_hun: Levenshtein distance computation at char level: %d (chars) %f (sec)" % (lenC,timeit.default_timer()-ti), file=sys.stderr)
            ti = timeit.default_timer()
            lvd_wl_alg = fastwer.compute(hyp, x, char_level=False)[0] + niw # Including inserted words
            if verbT: print("TIME WER_hun: Levenshtein distance computation at word level: %d (wrds) %f (sec)" % (lenW,timeit.default_timer()-ti), file=sys.stderr)
